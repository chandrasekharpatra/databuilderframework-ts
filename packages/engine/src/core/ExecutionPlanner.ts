...
export class ExecutionPlanner {
  ...
  createExecutionPlan(targetData: string[]): ExecutionPlan {
    ...
    if (isValid) {
      ...
      maxConcurrency = parallelExecutionLevels.length > 0 
        ? Math.max(...parallelExecutionLevels.map(level => level.length))
        : 0;
    }
  ...
  private calculateParallelLevels(executionOrder: DependencyNode[]): string[][] {
    ...
    while (remaining.size > 0) {
      ...
      for (const node of executionOrder) {
        ...
        const allDepsProcessed = Array.from(node.dependencies).every(dep => 
          processed.has(dep.meta.provides)
        );
