...
import { 
  ...
  ExecutionMode 
} from '../src/core/DataFlowEngine.js';
import { 
  CircularDependencyError, 
  MissingBuilderError 
} from '../src/core/ExecutionPlanner.js';
import { 
...
describe('DataFlowEngine', () => {
  ...
  describe('Builder Management', () => {
    ...
    test('should register multiple builders', () => {
      const builders = [
        new UserSourceBuilder(),
        new ConfigSourceBuilder(),
        new UserToProfileTransformer()
      ];
  ...
  describe('Error Handling', () => {
    test('should throw CircularDependencyError for circular dependencies', async () => {
      ...
      await expect(
        engine.executeWithOptions(context)
    ...
    test('should throw MissingBuilderError for missing dependencies', async () => {
      ...
      await expect(
        engine.executeWithOptions(context)
      ).rejects.toThrow(MissingBuilderError);
    });
    ...
    test('should throw BuilderExecutionError for failing builders', async () => {
      ...
      await expect(
        engine.executeWithOptions(context)
      ).rejects.toThrow(BuilderExecutionError);
    });
    ...
    test('should handle builder timeout', async () => {
      ...
      await expect(
        engine.executeWithOptions(context, { builderTimeout: 100 })
      ).rejects.toThrow();
    });
  ...
  describe('Complex Integration Scenarios', () => {
    ...
    test('should handle dynamic builder registration during execution', async () => {
      ...
      expect(plan2.parallelExecutionLevels).toEqual([
        ['user'],
        ['profile', 'userStats']
      ]);
    });
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle execution with no registered builders', async () => {
      await expect(
        engine.executeSimpleWithOptions(['nonexistent'])
      ).rejects.toThrow(MissingBuilderError);
    });
